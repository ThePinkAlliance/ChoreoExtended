...
public class ChoreoExtended {
  ...
  /**
   * Loads the split parts of the specified trajectory. Fails and returns null if
   * any of the parts
   * could not be loaded.
   *
   * <p>
   * This method determines the number of parts to load by counting the files that
   * match the
   * pattern "trajName.X.traj", where X is a string of digits. Let this count be
   * N. It then attempts
   * to load "trajName.1.traj" through "trajName.N.traj", consecutively counting
   * up. If any of these
   ...
   *
   * @param trajectory          The trajectory to follow. Use
   *                            Choreo.getTrajectory(String trajName) to load
   *                            this from the deploy directory.
   * @param poseSupplier        A function that returns the current field-relative
   *                            pose of the robot.
   * @param xController         A PIDController for field-relative X translation
   *                            (input: X error in meters,
   *                            output: m/s).
   * @param yController         A PIDController for field-relative Y translation
   *                            (input: Y error in meters,
   *                            output: m/s).
   * @param rotationController  A PIDController for robot rotation (input: heading
   *                            error in radians,
   *                            output: rad/s). This controller will have its
   *                            continuous input range set to -pi..pi by
   *                            ChoreoLib.
   * @param outputChassisSpeeds A function that consumes the target robot-relative
   *                            chassis speeds
   *                            and commands them to the robot.
   * @param mirrorTrajectory    If this returns true, the path will be mirrored to
   *                            the opposite side,
   *                            while keeping the same coordinate system origin.
   *                            This will be called every loop during the
   *                            command.
   * @param requirements        The subsystem(s) to require, typically your drive
   *                            subsystem only.
  ...
  public static Command choreoSwerveCommand(
      ...
      Consumer<DoubleSupplier> schedulerConsumer,
   ...
   * @param poseSupplier        A function that returns the current field-relative
   *                            pose of the robot.
   * @param controller          A ChoreoControlFunction to follow the current
   *                            trajectory state. Use
   *                            ChoreoCommands.choreoSwerveController(PIDController
   *                            xController, PIDController yController,
   *                            PIDController rotationController) to create one
   *                            using PID controllers for each degree of
   *                            freedom. You can also pass in a function with the
   *                            signature (Pose2d currentPose,
   *                            ChoreoTrajectoryState referenceState) -&gt;
   *                            ChassisSpeeds to implement a custom follower
   *                            (i.e. for logging).
   * @param outputChassisSpeeds A function that consumes the target robot-relative
   *                            chassis speeds
   *                            and commands them to the robot.
   ...
   *                           continuous input range set to -pi..pi by
   *                           ChoreoLib.
   * @return A ChoreoControlFunction to track ChoreoTrajectoryStates. This
   *         function returns
